...
export const PostWeddingView: React.FC = () => {
  ...
  return (
    <div className="min-h-screen py-16 px-4 max-w-6xl mx-auto space-y-20">
      ...
      >
        <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full glass-card text-xs text-gold-500 border-gold-glow uppercase tracking-widest">
          <Sparkles className="w-4 h-4" />
          <span>Düğün Hatırası Albümü</span>
        </div>

        <h1 className="font-serif text-4xl sm:text-6xl text-warm-700 font-light">
          {weddingConfig.groomName} <Heart className="inline w-8 h-8 text-rose-400 fill-rose-300" /> {weddingConfig.brideName}
        </h1>

        <p className="font-serif italic text-lg sm:text-xl text-warm-500 max-w-lg mx-auto leading-relaxed">
      ...
      <section className="space-y-8">
        ...
        {photos.length === 0 ? (
          <div className="text-center py-12 text-warm-500 glass-card max-w-md mx-auto p-6">
            <ImageIcon className="w-10 h-10 text-gold-400/60 mx-auto mb-2" />
            <p className="text-sm font-medium">Henüz yüklenmiş fotoğraf bulunmuyor.</p>
            <p className="text-xs text-warm-400 mt-1">İlk fotoğrafı yukarıdaki bölümden yükleyebilirsiniz!</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
            {photos.map((photo) => (
              <motion.div
                ...
                className="glass-card-gold rounded-2xl overflow-hidden border-gold-glow flex flex-col justify-between shadow-card hover:shadow-card-hover transition-shadow"
              >
                ...
                <div className="p-4 bg-white/80">
                  <p className="font-serif text-base text-warm-700 font-medium">
                    {photo.guest_name || 'İsimsiz Misafir'}
                  </p>
                  {photo.message && (
                    <p className="text-xs text-warm-500 italic mt-1">&quot;{photo.message}&quot;</p>
                  )}
                </div>
              </motion.div>
            ))}
      ...
      {messages.length > 0 && (
        <section className="space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {messages.map((item) => (
              <div key={item.id} className="glass-card p-5 border-gold-200/30">
                <div className="flex items-center justify-between mb-2">
                  <span className="font-serif text-lg text-warm-700 font-medium">{item.guest_name}</span>
